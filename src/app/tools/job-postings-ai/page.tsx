...
import JobPostingsAiForm from "@/components/tools/JobPostingsAiForm";

const FEATURE_KEY = "Job Postings.ai";

export default async function JobPostingsAiPage() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();

  // Middleware already redirects unauthenticated visitors to /login before
  // this ever renders, but guard anyway rather than assume.
  if (!user) {
    return (
  ...
  return (
    <AppShell title="Job Postings.ai">
      {hasAccess ? (
        ...
        <AccessDenied reason='The admin hasn’t granted you access to "Job Postings.ai" yet.' />
      )}
    </AppShell>
  );
}
